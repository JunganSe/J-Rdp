...
An example use case is when connection to a development server is done with single use ("burner") rdp files, and we want to automatically apply some settings before launching and then deleting the file.
...
Any derivative work based on this project must reference the original. ([github.com/JunganSe/J-Rdp](https://github.com/JunganSe/J-Rdp))
...
# Launching
No installation is needed, simply run the .exe file.
...
From here you can toggle profiles on or off, open the config file, view the log, and toggle log settings.\
![image](https://github.com/user-attachments/assets/54691c71-54d9-44f2-80dc-42e1a72b3aa2)
...
- `showLog`: Decides whether to show a window containing a log of program events. Can also be toggled from the tray menu.\
  Default if omitted: false
- `logToFile`: Decides whether to log program events to file. Can also be toggled from the tray menu.\
  Default if omitted: false
- `profiles`: (Described in the chapter below.)
...
## Logging
Logging to file can be enabled via the config or context menu, and is off by default.\
